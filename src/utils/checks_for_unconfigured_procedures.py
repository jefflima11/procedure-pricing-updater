...
from src.queries.checks_for_unconfigured_procedures_queries import insertProceduresInLogSQL, proceduresUnconfiguredLogSQL, deleteProceduresInLogSQL, insertProceduresInLogMatSQL, proceduresUnconfiguredLogMatSQL
import pandas as pd
...
def checksForUnconfiguredProceduresMat(df):
    ...
    try:
        con = get_connection()
        cur = con.cursor()
        cur.executemany(insertProceduresInLogMatSQL, data)
        con.commit()
        print('insert realizado!')
    except oracledb.Error as e:
        print('Erro ao executar insert:')
...
def exportForUnconfiguredProceduresMat():
    now = datetime.datetime.now()
    now_formated = now.strftime("%d%m%Y")
    ...
    try:
        con = get_connection()
        cur = con.cursor()
        cur.execute(proceduresUnconfiguredLogMatSQL)
        rows = cur.fetchall()
        columns = [col[0] for col in cur.description]
        df = pd.DataFrame(rows, columns=columns)
    except oracledb.Error as e:
